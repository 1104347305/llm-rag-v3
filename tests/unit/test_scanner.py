...
class TestScanner(unittest.IsolatedAsyncioTestCase):
    async def test_scan_prefers_data_when_project_root_is_passed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            files = await scan_markdown_files(root)
        self.assertEqual([path.name for path in files], ["kept.md"])

    async def test_scan_prefers_wiki_over_data(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "wiki").mkdir()
            (root / "data").mkdir()
            ...
            files = await scan_markdown_files(root)
        self.assertEqual([path.name for path in files], ["wiki-page.md"])
