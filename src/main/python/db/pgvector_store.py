...
class PgVectorStore:
    ...
    def upsert_vectors(self, project_id: str, chunks: list[Chunk]) -> None:
        """批量写入 / 更新向量（execute_values 批量操作）。"""
        valid = [(project_id, c.chunk_id, c.page_id, c.path, vector_to_str(c.vector))
    ...
    def search_vectors(self, project_id: str, query_vector: list[float], top_k: int = 100) -> list[tuple[str, float]]:
        """cosine 相似度搜索。
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    ...
                    limit %s
                """, (vector_to_str(query_vector), project_id, vector_to_str(query_vector), top_k))
                rows = cur.fetchall()
        return [(str(row[0]), float(row[1])) for row in rows]

    def delete_by_chunk_ids(self, project_id: str, chunk_ids: list[str]) -> None:
        """按 chunk_id 批量删除向量。"""
        if not chunk_ids:
            return
        with self._connect() as conn:
    ...
    def delete_by_paths(self, project_id: str, paths: list[str]) -> None:
...
def vector_to_str(vector: list[float]) -> str:
    """将 float 列表转为 pgvector halfvec 字符串格式。

    halfvec 范围 [-65504, 65504]，超出自动截断以保证写入安全。
    """
