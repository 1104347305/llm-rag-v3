...
class LocalStore:
    ...
    def save_index(
        ...
        if rebuild_sqlite or changed_paths is None or not self.has_sqlite_index(project_id):
    ...
    def load_index(self, project_id: str) -> tuple[list[Page], list[Chunk], list[GraphEdge]]:
        """从 JSON 快照加载完整索引。"""
    ...
    def save_manifest(self, project_id: str, manifest: dict[str, Any]) -> None:
        self.manifest_path(project_id).write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    def has_sqlite_index(self, project_id: str) -> bool:
        """检查 SQLite 索引是否存在。"""
        return self.sqlite_path(project_id).exists()

    def chunk_count(self, project_id: str) -> int | None:
        if not self.has_sqlite_index(project_id):
            return None
        with closing(self.connect(project_id)) as conn:
            row = conn.execute("select count(*) from chunks").fetchone()
        return int(row[0]) if row else 0

    def load_chunks_by_ids(self, project_id: str, chunk_ids: list[str]) -> list[Chunk]:
    ...
    def load_pages_by_ids(self, project_id: str, page_ids: list[str]) -> list[Page]:
    ...
    def load_chunks_for_pages(self, project_id: str, page_ids: list[str]) -> list[Chunk]:
        """加载指定页面的所有 chunk。"""
        ids = unique_strings(page_ids)
        if not ids:
            return []
    ...
    def load_edges_for_pages(self, project_id: str, page_ids: list[str]) -> list[GraphEdge]:
        """加载指定页面相关的所有图边。"""
        ids = unique_strings(page_ids)
        if not ids:
            return []
    ...
    def search_chunks_fts(self, project_id: str, query: str, top_k: int = 100) -> list[tuple[str, float]]:
        """FTS5 全文搜索 chunks，返回 [(chunk_id, bm25_score), ...]。"""
        fts_query = build_fts_query(query)
        if not fts_query or not self.has_sqlite_index(project_id):
            return []
        try:
            with closing(self.connect(project_id)) as conn:
                rows = conn.execute("""
